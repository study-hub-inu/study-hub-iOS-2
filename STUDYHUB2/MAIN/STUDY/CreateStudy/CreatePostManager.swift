...
final class PostManager {
  ...
  // 네트워킹 요청하는 함수
  func launchPost(methodType: String,
                  postData: CreateStudyRequest,
    ...
    getMethod(with: urlString, methodType: methodType,
              postData: postData, token: token, completion: completion)
  }
  
  // method 형식만 변수로 받으면 수정하기 가능할듯
  private func getMethod(with urlString: String,
                         methodType: String,
                         postData: CreateStudyRequest,
                         token: String,
                         completion: @escaping NetworkCompletion) {
